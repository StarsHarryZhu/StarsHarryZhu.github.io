...
- **峰谷计价**：内置 v4-pro / flash 系列官方价目（UTC 峰时段 01:00–04:00 与 06:00–10:00，谷 = 峰一半）；2026-08-23 起北京时间周六/周日全天谷价；2026-09-10 12:00 起 flash 系列启用新价目（峰 = 谷 × 2），历史会话按各自时刻取价不回溯。
- **flash 系列与别称**：主 id `deepseek-flash`；2026-09-10 12:00（北京时间）起 `deepseek-v4-flash`、`deepseek-v4-flash-vision-exp`（Vision at Flash Price）都是它的别称，一律引用 flash 价目（含峰谷与周末谷价）。`deepseek-chat` / `deepseek-reasoner` 自动按 flash 旧统一价计费；表外模型明确显示「不在内置价格表」而非瞎估。
